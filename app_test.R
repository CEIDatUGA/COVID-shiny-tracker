...
server <- function(input, output, session) {
  ...
  observeEvent( input$alltabs == 'world', 
                  ...
                  output$world_ui <- renderUI({
                    ...
                    sidebarLayout(
                      sidebarPanel(
                        ...
                        shiny::selectInput( "case_death_w", "Outcome",
                          c("Cases" = "Cases", "Deaths" = "Deaths"),
                        ),
                        shiny::selectInput("daily_tot_w", "Daily or Cumulative Numbers",
                          c("Daily" = "Daily", "Total" = "Total"),
                        ),
                        shiny::selectInput("absolute_scaled_w", "Absolute or scaled values",
        ...
        output$case_death_plot_world <- renderPlotly({
          w_plot_dat <- set_outcome(world_clean,input$case_death_w,input$daily_tot_w,input$absolute_scaled_w,input$xscale_w,input$count_limit_w,input$alltabs)
          #  get_plot_data_world()
  ...
  observeEvent( input$alltabs == 'us', 
  {
    output$us_ui <- renderUI({
      ...
      sidebarLayout(
        sidebarPanel(
          ...
          shinyWidgets::pickerInput(
            "state_selector",
            "Select States",            state_var,
            multiple = TRUE,
            options = list(`actions-box` = TRUE),
            selected = c("CA", "WA", "GA")
          ),
          #Shiny selectors below major picker input
          shiny::selectInput( "case_death",   "Outcome",
            c("Cases" = "Cases", "Deaths" = "Deaths", "Hospitalizations" = "Hospitalized"),
          ),
          shiny::selectInput("daily_tot", "Daily or total Numbers",
            c("Daily" = "Daily", "Total" = "Total"),
          ),
          
          shiny::selectInput( "absolute_scaled","Absolute or scaled values",
            c("Absolute number" = "actual", "Per 100K" = "scaled")
          ),
          
          # It would be nice if we could get the X Cases to auto-change to match the selector below
          shiny::selectInput(
            "xscale", "Set x-axis to calendar date or days since a specified Total number of cases",
            c("Calendar Date" = "x_time", "Days Since X Cases" = "x_count")
          ),
          sliderInput(  inputId = "count_limit", "Choose the Total number of cases at which to start graphs",
            min = 1,  max = 500, value = 10
          ),
          shiny::selectInput(  "yscale", "Y-scale",
            c("Linear" = "linear", "Logarithmic" = "logarithmic"),
            selected = "logarithmic")
        ),
        #end sidebar panel
    ...
    output$case_death_plot <- renderPlotly({
      us_plot_dat <- set_outcome(us_clean,input$case_death,input$daily_tot,input$absolute_scaled,input$xscale,input$count_limit,input$alltabs)
