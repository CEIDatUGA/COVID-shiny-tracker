...
#Combine nyt data to us_clean
us_nyt_clean <- us_nyt_clean %>% rename(nyt_total_cases = Total_Cases, nyt_total_deaths = Total_Deaths, nyt_daily_cases = Daily_Cases, nyt_daily_deaths = Daily_Deaths)
us_nyt_clean$Location <- state.abb[match(us_nyt_clean$Location, state.name)]
us_clean <- merge(us_clean, us_nyt_clean, all = TRUE)
...
server <- function(input, output, session) {
  ...
  ###########################################
  set_outcome <- function(plot_dat,case_death,daily_tot,absolute_scaled,xscale,count_limit,selected_tab,location_selector, data_source)
  {
