...
server <- function(input, output, session) {
  ...
  # uses plotly
  ###########################################
  make_plotly <- function(plot_list, location_selector, yscale, xscale, ylabel, outname, outtype, nytimes, selecttab)
  {
    ...
    {
      p_dat <- plot_dat %>%  dplyr::filter(source == 'Covid Tracker')
      ...
      p_dat <- plot_dat %>% dplyr::filter(source == 'NY Times')
      tooltip_text = paste(paste0("Location: ", p_dat$Location), paste0(tool_tip[1], ": ", p_dat$Date), paste0(tool_tip[ylabel+1],": ", p_dat[,outname]), sep ="\n") 
      ...
      pl <- pl3
            ...
            browser()
            #for those scenarios where NYT data is not available
            if (nytimes == "No" & selecttab == "us")
            {
              p_dat <- plot_dat %>% dplyr::filter(source == 'Covid Tracker')
            }
            if (selecttab == "world")
...
###########################################
  observeEvent( input$alltabs == 'world', 
        ...
        output$case_death_plot_world <- renderPlotly({
          #create plot
          pl <- make_plotly(plot_dat(), location_selector = input$country_selector, yscale = input$yscale_w, xscale = input$xscale_w, ylabel = 1, outname = 'outcome', outtype = input$case_death, nytimes = "No", input$alltabs)
  ...
  observeEvent( input$alltabs == 'us', 
  {
    ...
    output$case_death_plot <- renderPlotly({
      #create plot
      pl <- make_plotly(plot_dat(), location_selector = input$state_selector, yscale = input$yscale, xscale = input$xscale, ylabel = 1, outname = 'outcome',outtype = input$case_death, nytimes = input$nyt_data, input$alltabs)
    }) #end function making case/deaths plot
    
    #make the testing plot 
    output$testing_plot <- renderPlotly({
      #create plot
      pl <- make_plotly(plot_dat(), location_selector = input$state_selector, yscale = input$yscale, xscale = input$xscale, ylabel = 2, outname = 'test_outcome', outtype = input$case_death, nytimes = "No", input$alltabs)
    }) #end function making testing plot
    
    #make the fraction positive testing plot 
    output$testing_frac_plot <- renderPlotly({
      #create plot
      pl <- make_plotly(plot_dat(), location_selector = input$state_selector, yscale = "identity", xscale = input$xscale, ylabel = 3, outname = 'test_frac_outcome', outtype = input$case_death, nytimes = "No", input$alltabs)
    }) #end function making testing plot
