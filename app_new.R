...
get_data <- function()
{
  ...
  #add columns with a numeric location ID for plotting
  us_dat$locationid = as.factor(us_dat$location)
  world_dat$locationid = as.factor(world_dat$location)
                              ...
                              show_smoother, ylabel, outtype)
  {
    ...
    pl <- plotly::plot_ly(p_dat) %>% 
          plotly::add_trace(x = ~time, y = ~value, type = 'scatter', 
                                 mode = 'lines+markers', 
                                 linetype = ~source, symbol = ~locationid,
                                 line = list(width = linesize), text = tooltip_text, 
                                 color = ~locationid, colors = brewer.pal(ncols, "Dark2")) %>%
