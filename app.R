...
#################################
#load latest data once when shiny app is run
#################################

datafiles <- list.files("data",pattern = "clean_data", full.names = T) 
latest_datafile <- datafiles[which.max(file.mtime(datafiles))]
all_dat <- readRDS(latest_datafile)
...
server <- function(input, output, session) {
  ...
  output$source_selector = renderUI({
    shinyWidgets::pickerInput("source_selector", "Select Sources (see 'About' tab for details)", us_source_var, multiple = TRUE,options = list(`actions-box` = TRUE), selected = c("JHU") )
  })
              ...
              tabPanel( title = "US Counties", value = "county",
                        sidebarLayout(
                          sidebarPanel(
                            ...
                            sliderInput(inputId = "x_limit_c", "Select a date or outcome value from which to start the plots.", min = mindate,  max = Sys.Date(), value = defaultdate ),
