...
get_data <- function()
{
  ...
  county_nyt_clean <- clean_counties(county_nyt, county_popsize) %>%
    ungroup() %>% select(-c(Location, state_abr))
  #reformat to long
  ...
  county_jhu_clean <- inner_join(us_jhu_total, county_popsize) %>% 
    select(-c(Population_Size, FIPS, state_abr))
  #Use us_jhu_total to create both the county and state level datasets 
  #Pull county data and reformat to long
  #county_jhu_clean <- us_jhu_clean %>% select(-c(Population_Size, FIPS)) %>%
  ...
  county_dat <- county_dat %>% 
                rename(date = Date, location = county_name, populationsize = pop_size, totalcases = Total_Cases, totaldeaths = Total_Deaths, dailycases = Daily_Cases, dailydeaths = Daily_Deaths)

  #reorder columns
  #county_dat <- county_dat[c("source","location","state", "populationsize","date","Total_Cases","Total_Deaths","Daily_Cases","Daily_Deaths")]
...
server <- function(input, output, session) {
                              ...
                              show_smoother, ylabel, outtype)
  {
    ...
    if (current_tab == "county")
    {
      ...
      county_dat <- county_dat <- pivot_longer(county_dat, cols = c(-source, -location, -date, -state, -populationsize), names_to = "variable", values_to = "value")
